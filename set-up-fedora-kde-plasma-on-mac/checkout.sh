...
branch_off_upstream_branch() {
  ...
  UPSTREAM_VERSION=$(rpmspec --query --queryformat='%{VERSION}' --srpm "$TMP_DIR/${project}.spec" 2>/dev/null)
  rm -rf "$TMP_DIR"
...
set_fork_version() {
  ...
  case "$project" in
    ...
    plasma-login-manager)
      ...
      BRANCH="customize/v$PACKAGE_LATEST_VERSION"
      UPSTREAM_BRANCH="refs/upstream/f$FEDORA_VERSION"
      branch_off_upstream_branch "$project_dir" "$BRANCH" "$UPSTREAM_BRANCH" "$PACKAGE_LATEST_VERSION" || return 1
      mark_build_branch "$project_dir" "$BRANCH" "$FEDORA_VERSION" || return 1

      BRANCH_COMMIT=$(git -C "$project_dir" rev-parse "$UPSTREAM_BRANCH" 2>/dev/null)
