...
Events:
  - Item moved to trash  → plays "file-trash"
...
class TrashMonitor(QObject):
    ...
    def on_files_added(self, url: str) -> None:
        ...
        if new_count > self._trash_count:
            ...
            if sound_file := self._sound_files.get("file-trash"):
    ...
    def on_files_removed(self, urls: list) -> None:
        ...
        if not trash_urls:
            return
        new_count = trash_item_count()
        prev_count = self._trash_count
        self._trash_count = new_count
        # Only play the empty-trash sound when all remaining items were removed
        # (i.e. trash is now empty). Restoring items leaves the count > 0.
        if new_count == 0 and prev_count > 0:
            ...
            log.info("Item(s) removed from trash (count: %d → %d).", prev_count, new_count)
